...
describe('fetchSubject', () => {
  it('should pass on a direct reference to Tripledoc', () => {
    ...
    expect(tripledoc.fetchDocument.mock.calls[0][0]).toBe('https://arbitrary.doc/resource.ttl#subject');
  });
  ...
  it('should not share caches over different virtual Subjects', () => {
    const tripledoc = jest.requireMock('tripledoc');
    const virtualSubject1 = selectSubjectFromRef('https://arbitrary.doc/resource.ttl#subject');
    const virtualSubject2 = selectSubjectFromRef('https://arbitrary.doc/resource.ttl#subject');
    ...
    expect(tripledoc.fetchDocument.mock.calls[0][0]).toBe('https://arbitrary.doc/resource.ttl#subject');
    expect(tripledoc.fetchDocument.mock.calls[1][0]).toBe('https://arbitrary.doc/resource.ttl#subject');
  });
});
