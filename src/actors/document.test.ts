...
describe('fetchDocument', () => {
  it('should pass on a direct reference to Tripledoc', () => {
    ...
    const virtualDocument = locateDocByRef('https://arbitrary.doc/resource.ttl');

    fetchDocument(virtualDocument);

    expect(tripledoc.fetchDocument.mock.calls.length).toBe(1);
    expect(tripledoc.fetchDocument.mock.calls[0][0]).toBe('https://arbitrary.doc/resource.ttl');
  });
  ...
  it('should not share caches over different virtual Documents', () => {
    const tripledoc = jest.requireMock('tripledoc');
    const virtualDocument1 = locateDocByRef('https://arbitrary.doc/resource.ttl');
    const virtualDocument2 = locateDocByRef('https://arbitrary.doc/resource.ttl');
    ...
    expect(tripledoc.fetchDocument.mock.calls[0][0]).toBe('https://arbitrary.doc/resource.ttl');
    expect(tripledoc.fetchDocument.mock.calls[1][0]).toBe('https://arbitrary.doc/resource.ttl');
  });
});
...
describe('ensureForRef', () => {
  it('should return an existing Document if referred to', () => {
    ...
    const mockSubject = {};
    const virtualSubject = selectSubjectFromRef('https://arbitrary.doc/resource.ttl#subject');
